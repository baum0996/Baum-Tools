...
class TexturePackScannerPage:
    ...
    def _render(self) -> None:
        ...
        self.console.print()
        self.console.print(Panel.fit("Texture Pack Scanner", border_style="cyan"))
        ...
        source_table.add_column("Name", style="bold cyan", no_wrap=True)
        ...
        menu.add_column(justify="right", style="bold cyan")
        menu.add_column(style="white")
        menu.add_row("[1]", "Select Folder")
        menu.add_row("[2]", "Select ZIP Archive")
        menu.add_row("[3]", "Scan")
        menu.add_row("[4]", "Hash Generator")
        menu.add_row("[5]", "Export Report")
        menu.add_row("[6]", "Deep Scan")
        menu.add_row("[7]", "Extension Statistics")
        menu.add_row("[8]", "Back")
        self.console.print(Panel(menu, title="Actions", border_style="blue"))
        self.console.print()

    def _select_folder(self) -> None:
        self._clear_if_enabled()
        self.console.print(Panel("Opening native folder picker...", border_style="cyan"))
        try:
            selected = self.file_picker.select_folder()
        except Exception as exc:
            self._logger.exception("Folder selection failed.")
            self._message("Folder Selection Failed", str(exc), "red")
            return

        if selected is None:
            self._message("Folder Selection", "No folder was selected.", "yellow")
            return

        self.selected_folder = selected
        self.selected_zip = None
        self.source_label = "Folder"
        self._message("Folder Selected", str(selected), "green")

    def _select_zip(self) -> None:
        self._clear_if_enabled()
        self.console.print(Panel("Opening native file picker...", border_style="cyan"))
        try:
            ...
            self._message("ZIP Selection Failed", str(exc), "red")
            return

        if selected is None:
            self._message("ZIP Selection", "No ZIP archive was selected.", "yellow")
            return

        self.selected_zip = selected
        self.selected_folder = None
        self.source_label = "ZIP Archive"
        self._message("ZIP Archive Selected", str(selected), "green")
    ...
    def _scan(self) -> None:
        try:
            if self.selected_folder is not None:
                ...
                self._message("Scan", "Select a folder or ZIP archive first.", "yellow")
                return
        except BadZipFile:
            ...
            self._message("Scan Failed", str(exc), "red")
            return
    ...
    def _render_report(self, source: str, analysis: TexturePackAnalysis) -> None:
        self._clear_if_enabled()
        self.console.print()
        self.console.print(Panel.fit("Texture Pack Analysis", border_style="cyan"))
        self.console.print(f"[bold cyan]Source:[/bold cyan] [white]{source}[/white]")
        self.console.print()

        counts = Table(box=box.SIMPLE_HEAVY, border_style="blue", show_header=False)
        counts.add_column("Type", style="bold cyan", no_wrap=True)
        counts.add_column("Count", justify="right", style="white")
        ...
        executable_table = Table(
            ...
            box=box.SIMPLE_HEAVY,
            border_style="red" if analysis.executable_files else "green",
            show_lines=False,
        )
        executable_table.add_column("Path", style="white")
        ...
        style = "red" if analysis.executable_files else "green"
        self.console.print(Panel(status, title="Status", border_style=style))
        self.console.print()

        if not analysis.executable_files:
            self.console.print("[green]No executable files detected.[/green]")
            self.console.print()

        self._wait_for_back()

    def _coming_soon(self) -> None:
        self._message("Coming Soon", "Coming Soon", "yellow")
